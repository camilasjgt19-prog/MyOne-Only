...
Para que no se te rompa nada, lo más fácil es colocar la función de los corazones al final y llamar a esa función dentro de tu función cambiar.

Aquí tienes cómo debe quedar tu archivo script.js completo. He integrado los corazones y las flores dentro de tu código para que solo tengas que borrar todo el contenido de tu script.js y pegar este:
...
function cambiar() {
    ...
    if (elementoTexto && elementoImagen) {
        ...
        // LANZAR CORAZONES Y FLORES AL HACER CLIC
        lanzarDecoracion();
    }
}

// ESTA ES LA FUNCIÓN NUEVA QUE DEBES PONER AL FINAL
function lanzarDecoracion() {
    ...
    for (let k = 0; k < 6; k++) {
        ...
        el.style.top = "100vh"; // Empiezan abajo
        ...
        // Animarlos hacia arriba
        setTimeout(() => {
            el.style.top = "-10vh"; // Suben hasta arriba
            el.style.transform = `translateX(${(Math.random() - 0.5) * 200}px) rotate(360deg)`;
            el.style.opacity = "0";
        }, 50);

        // Borrarlos cuando terminen de subir
        setTimeout(() => {
            el.remove();
        }, 3000);
    }
